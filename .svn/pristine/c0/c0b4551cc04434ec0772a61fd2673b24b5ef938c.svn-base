<template>
  <div>
    <h2>Taurus服务调用演示页面</h2>
    <button v-on:click="writeCookie">write cookie</button>
    <button v-on:click="readCookie">read cookie</button>
    <button v-on:click="removeCookie">remove cookie</button>
    <button v-on:click="callService">call service</button>
  </div>
  <div>Cookie信息如下: </div>
  <div>
    <div><span>SEC_AUTH_TOKEN_KEY: </span>{{ sec_auth_token_key }}</div>
    <div><span>SEC_AUTH_TENANT_KEY: </span>{{ sec_auth_tenant_key }}</div>
    <div><span>SEC_AUTH_ROLETYPE_KEY: </span>{{ sec_auth_roletype_key }}</div>
    <div><span>SEC_AUTH_ZONE_KEY: </span>{{ sec_auth_zone_key }}</div>
  </div>
</template>
